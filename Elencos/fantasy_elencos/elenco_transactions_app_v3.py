...
with st.form("tx_form", clear_on_submit=True):
    ...
    for i in range(int(n_items)):
        ...
        with cols[1]:
            roster_type = st.selectbox(f"Roster origem {i+1}", ["MAIN", "DEV"], key=f"from_rt_{i}") if item_type == "player" else st.selectbox(f"Roster origem {i+1}", ["MAIN"], key=f"from_rt_{i}")
            if item_type == "player":
                asset_choice = st.selectbox(f"Ativo {i+1}", [player_labels[pid] for pid in player_choices] if player_choices else ["Sem jogadores disponíveis"], key=f"asset_{i}")
            else:
        ...
        if item_type == "player":
            pid = int(str(asset_choice).split(" - ")[0])
        else:
...
if submitted:
    try:
        ...
        items = data.get("transaction_times", pd.DataFrame())
        tx_id = get_next_id(txs, "transaction_id")
        to_team_id = int(teams.loc[teams["team_name"] == to_team_name, "team_id"].iloc[0])
        tx_row = {"transaction_id": tx_id, "transaction_type": tx_type, "transaction_date": pd.to_datetime(tx_date).to_pydatetime(), "season": effective_season, "from_team_id": from_team_id, "to_team_id": to_team_id, "initiated_by": initiated_by, "status": status, "notes": notes}
        ...
        for idx, item in enumerate(item_rows):
            payload.append({"transaction_id": tx_id, "item_id": next_item_id + idx, "item_type": item["item_type"], "asset_id": item["asset_id"], "from_team_id": from_team_id, "to_team_id": to_team_id, "from_roster_type": item["from_roster_type"], "to_roster_type": item["to_roster_type"], "effective_season": effective_season, "item_notes": item["item_notes"]})
        append_transaction(str(DEFAULT_FILE), tx_row, payload)
        update_rosters(str(DEFAULT_FILE), tx_row, payload)
...
with st.expander("Diagnóstico de carregamento", expanded=False):
    st.json({"players": len(data["players"]), "teams": len(data["teams"]), "roster": len(data["roster"]), "development": len(data["development"]), "picks": len(data["picks"]), "fines": len(data["fines"]), "transactions": len(transactions), "transaction_times": len(transaction_times), "time_selecionado": selected_team_name, "temporadas_visiveis": [SEASON_LABELS[s] for s in visible_seasons]})
