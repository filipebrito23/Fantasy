...
st.caption("Versão v2: transações aplicadas ao pipeline antes das visões do roster e development.")
...
def apply_transactions(roster_df: pd.DataFrame, transactions_times: pd.DataFrame, team_id: int, roster_type: str) -> pd.DataFrame:
    if roster_df.empty or transactions_times.empty:
        return roster_df
    out = roster_df.copy()
    rel = transactions_times.copy()
    if "effective_season" in rel.columns:
        rel = rel[rel["effective_season"].astype(str).str.len() > 0]
    ...
    if rel.empty:
        return out
    if "player_id" in out.columns and "asset_id" in rel.columns:
        removed = set(rel.loc[rel["to_team_id"].ne(team_id), "asset_id"].dropna().tolist())
        if removed:
            out = out[~out["player_id"].isin(removed)]
        added = rel.loc[rel["to_team_id"].eq(team_id), "asset_id"].dropna().tolist()
        if added:
