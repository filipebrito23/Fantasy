...
ABA_TESTE = None
PROCESSAR_APENAS_PRIMEIRA_LINHA = False
...
def testar_escrita(ws):
    valor_antes = ws.acell("E2").value
    log(f"Teste escrita em {ws.title}!E2 | antes={valor_antes}")
    ws.update_acell("E2", "999")
    time.sleep(2)
    valor_depois = ws.acell("E2").value
    log(f"Teste escrita em {ws.title}!E2 | depois={valor_depois}")
...
def processar_aba(ws):
    ...
    for i, row in df.iterrows():
        ...
        if all(str(stats.get(col, "")).strip() == "" for col in COLUNAS_STATS):
            log(f"Sem stats para {jogador}; pulando escrita.")
            continue
