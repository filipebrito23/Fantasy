    ...
    for i, row in df.iterrows():
       ...
       # --- Agrupar escritas por linha (batch_update) ---
        ...
        for stat_col in COLUNAS_STATS:
            ...
            log(f"Enfileirar escrita: {worksheet.title}!{cell_a1} = {val!r}")
            updates.append({
                "range": cell_a1,
                "values": [[val]]
            })
        ...
        if updates:
            try:
                worksheet.batch_update(updates)  # uma requisição por linha
                log(f"Linha {i+2}: escrita em lote realizada.")
                if MAIN_DEBUG:
                    time.sleep(0.5)  # pequeno delay após a escrita da linha
            except Exception as exc:
                log(f"Erro ao fazer batch_update para linha {i+2}: {exc}")
