...
def apply_trade_totals(current, out_rows, in_rows):
    out_sum, in_sum = trade_package_summary(out_rows, in_rows)
    return {
        "pts": current["pts"] + (in_sum["pts"] - out_sum["pts"]),
        "trb": current["trb"] + (in_sum["trb"] - out_sum["trb"]),
        "ast": current["ast"] + (in_sum["ast"] - out_sum["ast"]),
        "stl": current["stl"] + (in_sum["stl"] - out_sum["stl"]),
        "blk": current["blk"] + (in_sum["blk"] - out_sum["blk"]),
        "three_p": current["three_p"] + (in_sum["three_p"] - out_sum["three_p"]),
        "tov": current["tov"] + (in_sum["tov"] - out_sum["tov"]),
    }
...
def get_scenarios_for_team(scenarios_df, team_id):
    if scenarios_df.empty or 'team_id' not in scenarios_df.columns:
        return pd.DataFrame()
...
def get_scenario_rows(df, scenario_id):
    if df.empty or 'scenario_id' not in df.columns:
        return pd.DataFrame()
    out = df.copy()
    out['scenario_id'] = pd.to_numeric(out['scenario_id'], errors='coerce')
    return out[out['scenario_id'] == scenario_id].copy()


def enrich_scenario_roster(df, players, order_col=None):
    if df.empty:
        return pd.DataFrame()
    dc, pc = team_cols(df), team_cols(players)
    if dc['player_id'] is None or pc['player_id'] is None:
        return df.copy()
    out = df.copy()
    out[dc['player_id']] = pd.to_numeric(out[dc['player_id']], errors='coerce')
    p = players.copy()
    p[pc['player_id']] = pd.to_numeric(p[pc['player_id']], errors='coerce')
    merged = out.merge(p, left_on=dc['player_id'], right_on=pc['player_id'], how='left', suffixes=('', '_player'))
    if order_col and order_col in merged.columns:
        merged = merged.sort_values(order_col)
    return merged


def scenario_team_totals(scenario_lineup_df):
    totals = {c: np.nan for c in STAT_COLS}
    if scenario_lineup_df.empty:
        return totals
    for c in STAT_COLS:
        if c in scenario_lineup_df.columns:
            totals[c] = pd.to_numeric(scenario_lineup_df[c], errors='coerce').sum()
    return totals


def build_virtual_teams_calc(teams_calc, team_id, scenario_totals):
    vt = teams_calc.copy()
    mask = vt['team_id'] == team_id
    for c in STAT_COLS:
        if c in vt.columns and c in scenario_totals:
            vt.loc[mask, c] = scenario_totals[c]
    return vt


def scenario_matchup_delta(team_id, current, scenario_totals, teams_calc):
    opps = teams_calc[(teams_calc['team_id'] != team_id) & (teams_calc['team_id'] != 15)].dropna(subset=STAT_COLS).copy()
    if opps.empty:
        return np.nan, np.nan, np.nan
    old_m = float(np.mean([matchup_score(pd.Series(current), opp) for _, opp in opps.iterrows()]))
    new_m = float(np.mean([matchup_score(pd.Series(scenario_totals), opp) for _, opp in opps.iterrows()]))
...
st.title("Fantasy NBA Dashboard v16 cenários")
...
if view == "Resumo":
    ...
    team_scenarios = get_scenarios_for_team(scenarios, team_id)
    if team_scenarios.empty:
        st.info("Nenhum cenário cadastrado para este time.")
    else:
        meta_cols = [c for c in ["scenario_id", "scenario_name", "team_id", "move_type", "description", "created_at", "is_active"] if c in team_scenarios.columns]
        st.dataframe(team_scenarios[meta_cols], use_container_width=True)

        if 'scenario_id' not in team_scenarios.columns:
            st.warning("A tabela scenarios não possui scenario_id.")
            st.stop()

        options = team_scenarios['scenario_id'].dropna().tolist()
        selected_scenario_id = st.selectbox(
            "Selecionar cenário",
            options,
            format_func=lambda x: f"{int(x)} - " + str(team_scenarios.loc[team_scenarios['scenario_id'] == x, 'scenario_name'].iloc[0]) if 'scenario_name' in team_scenarios.columns and not team_scenarios.loc[team_scenarios['scenario_id'] == x].empty else str(x)
        )

        scenario_meta = team_scenarios[team_scenarios['scenario_id'] == selected_scenario_id].head(1)
        row1, row2, row3 = st.columns(3)
        with row1:
            st.metric("Scenario ID", int(selected_scenario_id) if pd.notna(selected_scenario_id) else "NA")
        with row2:
            move_type_val = scenario_meta['move_type'].iloc[0] if ('move_type' in scenario_meta.columns and not scenario_meta.empty) else 'NA'
            st.metric("Move type", move_type_val)
        with row3:
            active_val = scenario_meta['is_active'].iloc[0] if ('is_active' in scenario_meta.columns and not scenario_meta.empty) else 'NA'
            st.metric("Ativo", active_val)

        scen_lineup_raw = get_scenario_rows(scenario_lineup, selected_scenario_id)
        scen_bench_raw = get_scenario_rows(scenario_bench, selected_scenario_id)
        scen_moves = get_scenario_rows(scenario_moves, selected_scenario_id)

        lineup_order_col = first_col(scen_lineup_raw, ['slot', 'slot_order', 'order']) if not scen_lineup_raw.empty else None
        bench_order_col = first_col(scen_bench_raw, ['bench_order', 'benchorder', 'order']) if not scen_bench_raw.empty else None
        scen_lineup_view = enrich_scenario_roster(scen_lineup_raw, players, lineup_order_col)
        scen_bench_view = enrich_scenario_roster(scen_bench_raw, players, bench_order_col)

        base_current = {c: pd.to_numeric(sel_row[c], errors='coerce') if c in sel_row.index else np.nan for c in STAT_COLS}
        scenario_totals = scenario_team_totals(scen_lineup_view)
        impact_df = compare_trade_teams(base_current, scenario_totals)
        old_m, new_m, delta_m = scenario_matchup_delta(team_id, base_current, scenario_totals, teams_calc)

        scen_tabs = st.tabs(["Resumo do cenário", "Titulares do cenário", "Banco do cenário", "Moves", "Needs e Matchup"])

        with scen_tabs[0]:
            m1, m2, m3 = st.columns(3)
            m1.metric("PTS base", f"{base_current['pts']:.2f}" if pd.notna(base_current['pts']) else 'NA')
            m2.metric("PTS cenário", f"{scenario_totals['pts']:.2f}" if pd.notna(scenario_totals['pts']) else 'NA')
            m3.metric("Delta PTS", f"{scenario_totals['pts'] - base_current['pts']:+.2f}" if pd.notna(base_current['pts']) and pd.notna(scenario_totals['pts']) else 'NA')
            st.markdown("### Base x Cenário")
            st.dataframe(impact_df, use_container_width=True)
            st.plotly_chart(px.bar(impact_df, x="Categoria", y="Delta", color="Delta", title="Impacto do cenário por categoria"), use_container_width=True)

        with scen_tabs[1]:
            if scen_lineup_view.empty:
                st.info("Nenhum titular cadastrado para este cenário.")
            else:
                cols = [c for c in ["player_name", "position", "slot", "pts", "trb", "ast", "stl", "blk", "three_p", "tov", "fantasy_value"] if c in scen_lineup_view.columns]
                st.dataframe(display_table(scen_lineup_view, cols), use_container_width=True)

        with scen_tabs[2]:
            if scen_bench_view.empty:
                st.info("Nenhum banco cadastrado para este cenário.")
            else:
                cols = [c for c in ["player_name", "position", "bench_order", "pts", "trb", "ast", "stl", "blk", "three_p", "tov", "fantasy_value"] if c in scen_bench_view.columns]
                st.dataframe(display_table(scen_bench_view, cols), use_container_width=True)

        with scen_tabs[3]:
            if scen_moves.empty:
                st.info("Nenhum move cadastrado para este cenário.")
            else:
                st.dataframe(scen_moves, use_container_width=True)

        with scen_tabs[4]:
            scen_virtual_teams = build_virtual_teams_calc(teams_calc, team_id, scenario_totals)
            scenario_needs = roster_needs(scen_lineup_view, scen_virtual_teams) if not scen_lineup_view.empty else pd.DataFrame()
            a, b, c = st.columns(3)
            a.metric("Matchup médio base", f"{old_m:.2f}" if pd.notna(old_m) else 'NA')
            b.metric("Matchup médio cenário", f"{new_m:.2f}" if pd.notna(new_m) else 'NA')
            c.metric("Delta matchup", f"{delta_m:+.2f}" if pd.notna(delta_m) else 'NA')
            if not scenario_needs.empty:
                st.markdown("### Needs do cenário")
                st.dataframe(scenario_needs, use_container_width=True)
                st.plotly_chart(px.bar(scenario_needs, x="Categoria", y="need", color="relative_strength", title=f"Needs do cenário - {sel_team}"), use_container_width=True)
