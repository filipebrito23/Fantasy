...
def classify_value_tiers(team_players):
    team_players = team_players.copy()
    team_players["fantasy_value"] = (
        team_players.get("pts", 0).fillna(0)
        + team_players.get("trb", 0).fillna(0)
        + team_players.get("ast", 0).fillna(0)
        + team_players.get("stl", 0).fillna(0) * 1.5
        + team_players.get("blk", 0).fillna(0) * 1.5
        + team_players.get("three_p", 0).fillna(0)
        - team_players.get("tov", 0).fillna(0)
    )
    if len(team_players) >= 4:
        try:
            team_players["tier_valor"] = pd.qcut(team_players["fantasy_value"], q=min(4, team_players["fantasy_value"].nunique()), labels=False, duplicates="drop")
            max_tier = team_players["tier_valor"].max()
            team_players["tier_valor"] = team_players["tier_valor"].apply(lambda x: f"Tier {int(max_tier - x + 1)}" if pd.notna(x) else "NA")
        except Exception:
            team_players["tier_valor"] = "Tier único"
    else:
        team_players["tier_valor"] = "Tier único"
    return team_players
...
def roster_needs(team_players, teams_df):
    ...
    return pd.DataFrame(rows).sort_values("Necessidade", ascending=False)
...
def rank_targets(team_players, pool, team_id, teams_df, same_position=True, same_tier=True, top_n=15):
    team_players = classify_value_tiers(team_players)
    pool = pool.copy()
    pool["fantasy_value"] = (
        pool.get("pts", 0).fillna(0)
        + pool.get("trb", 0).fillna(0)
        + pool.get("ast", 0).fillna(0)
        + pool.get("stl", 0).fillna(0) * 1.5
        + pool.get("blk", 0).fillna(0) * 1.5
        + pool.get("three_p", 0).fillna(0)
        - pool.get("tov", 0).fillna(0)
    )
    current = team_stats(team_players)
    ...
    tier_gap = max((team_players["fantasy_value"].max() - team_players["fantasy_value"].min()) / 4, 1)
        ...
        for _, out_row in team_players.iterrows():
            if same_position and "position" in in_row.index and "position" in out_row.index:
                if str(in_row.get("position")) != str(out_row.get("position")):
                    continue
            if same_tier:
                if abs(safe_num(in_row.get("fantasy_value", np.nan)) - safe_num(out_row.get("fantasy_value", np.nan))) > tier_gap:
                    continue
            ...
            impact_sorted = sorted(impact_map.items(), key=lambda kv: kv[1], reverse=True)
            biggest_gain = impact_sorted[0][0] if impact_sorted else "NA"
            ...
            rows.append({
                ...
                "tier_out": out_row.get("tier_valor", "NA"),
                "valor_out": out_row.get("fantasy_value", np.nan),
                "valor_in": in_row.get("fantasy_value", np.nan),
                "antes": old_m,
                "depois": new_m,
                "delta": new_m - old_m if pd.notna(old_m) and pd.notna(new_m) else np.nan,
                "fit_elenco": best_need_fit,
                "maior_ganho": biggest_gain,
...
st.title("Fantasy NBA Dashboard v4.4")
...
if view == "Visão geral":
    ...
    display_cols = [c for c in ["player_name","position","fantasy_value","tier_valor"] + STAT_COLS if c in team_players.columns]
    base_df = team_players[display_cols].copy()
    if "fantasy_value" in base_df.columns:
        base_df = base_df.sort_values("fantasy_value", ascending=False)
    ...
    pool = classify_value_tiers(pool) if not pool.empty else pool
    ...
    impact_df["Direção"] = impact_df.apply(lambda r: "Melhora" if (r["Categoria"] == "TOV" and r["Delta"] < 0) or (r["Categoria"] != "TOV" and r["Delta"] > 0) else "Piora" if pd.notna(r["Delta"]) and r["Delta"] != 0 else "Neutro", axis=1)
    ...
    same_tier = st.checkbox("Apenas nível parecido de valor", value=True)
    pos_filter = st.selectbox("Filtrar alvo por posição", ["Todas"] + sorted(team_players["position"].dropna().astype(str).unique().tolist()) if "position" in team_players.columns else ["Todas"])
    ...
    if pos_filter != "Todas" and "position" in pool.columns:
        pool = pool[pool["position"].astype(str) == pos_filter]
    ranked = rank_targets(team_players, pool, team_id, teams, same_position=same_position, same_tier=same_tier, top_n=top_n)
    ...
    selected_in = st.selectbox("Detalhar alvo", ranked["jogador_entrando"].drop_duplicates().tolist())
    selected_row = ranked[ranked["jogador_entrando"] == selected_in].iloc[0]

    k1, k2, k3, k4 = st.columns(4)
    k1.metric("Delta matchup", f"{selected_row['delta']:+.2f}")
    k2.metric("Fit elenco", f"{selected_row['fit_elenco']:+.2f}" if pd.notna(selected_row['fit_elenco']) else "NA")
    k3.metric("Maior ganho", str(selected_row["maior_ganho"]))
    k4.metric("Tier saindo", str(selected_row["tier_out"]))

    impact_cols = ["delta_pts","delta_trb","delta_ast","delta_stl","delta_blk","delta_3pt","delta_tov"]
    impact_labels = ["PTS","TRB","AST","STL","BLK","3PT","TOV"]
    fig = go.Figure()
    fig.add_trace(go.Bar(x=impact_labels, y=[selected_row[c] for c in impact_cols], marker_color="#4C78A8", name="Impacto"))
    fig.update_layout(title="Leitura de impacto da sugestão selecionada", height=420)
    st.plotly_chart(fig, use_container_width=True)
    ...
    for player_out in top_ranked["jogador_saindo"].dropna().astype(str).unique():
        ...
        fig2.add_trace(go.Bar(name=player_out, x=d["jogador_entrando"], y=d["delta"]))
    fig2.update_layout(title="Top ganhos de matchup win", barmode="group", height=500, xaxis_title="Alvo", yaxis_title="Delta")
