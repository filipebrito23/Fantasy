...
def roster_needs(team_players, teams_df):
    if team_players.empty:
        return pd.DataFrame()
    stat_cols = [c for c in AVG_COLS if c in teams_df.columns]
    if not stat_cols:
        return pd.DataFrame()
    team_means = {}
    for c in stat_cols:
        src = c.replace('_avg', '')
        team_means[c] = pd.to_numeric(team_players[src], errors='coerce').mean() if src in team_players.columns else np.nan
    rows = []
    valid = teams_df.dropna(subset=stat_cols).copy()
    for c in stat_cols:
        val = team_means.get(c, np.nan)
