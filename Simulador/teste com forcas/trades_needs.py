...
def need_metrics(team_players):
    ...
    if not cols:
        return pd.DataFrame()
    vals = team_players[cols].copy()
    rel_strength = vals.mean(numeric_only=True)
    rel_strength = (rel_strength - vals.mean(numeric_only=True).mean()) / (vals.std(numeric_only=True).mean() if vals.std(numeric_only=True).mean() not in [0, np.nan] else 1)
    rel_strength = rel_strength.replace([np.inf, -np.inf], np.nan).fillna(0)
    abs_need = 1 / (vals.mean(numeric_only=True).replace(0, np.nan).abs())
    abs_need = abs_need.replace([np.inf, -np.inf], np.nan).fillna(0)
    df = pd.DataFrame({"stat": cols, "value": vals.mean(numeric_only=True).values, "relative_strength": [float(rel_strength.get(c, 0)) for c in cols], "need": [float(abs_need.get(c, 0)) for c in cols]})
    df["need_score"] = df["need"].rank(ascending=False, method="dense")
    return df.sort_values(["need", "relative_strength"], ascending=[False, True])
