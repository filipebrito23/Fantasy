...
def roster_needs(team_players, teams_df):
    ...
    for c in stat_cols:
        src = c.replace('_avg', '')
        team_sums[c] = pd.to_numeric(team_players[src], errors='coerce').sum() if src in team_players.columns else np.nan #mudei para a soma, mas mantive a media no nome
    rows = []
...
if view == "Resumo":
    ...
    out_sum, in_sum = trade_package_summary(out_rows, in_rows)
    pkg = pd.DataFrame({"Categoria": [STAT_LABELS[c] for c in STAT_COLS], "Saindo": [out_sum[c] for c in STAT_COLS], "Entrando": [in_sum[c] for c in STAT_COLS]})
    pkg["Delta"] = pkg["Entrando"] - pkg["Saindo"]
    st.markdown("### Pacote saindo vs entrando")
    st.dataframe(pkg, use_container_width=True)
    pkg_long = pd.DataFrame({
        "Categoria": [STAT_LABELS[c] for c in STAT_COLS] + [STAT_LABELS[c] for c in STAT_COLS],
        "Tipo": ["Saindo"] * len(STAT_COLS) + ["Entrando"] * len(STAT_COLS),
        "Valor": [out_sum[c] for c in STAT_COLS] + [in_sum[c] for c in STAT_COLS],
    })
    fig_pkg = px.bar(pkg_long, x="Categoria", y="Valor", color="Tipo", barmode="group", title="Pacote saindo vs entrando")
