...
if view == "Resumo":
    ...
    pkg_rows = []
    for c in STAT_COLS:
        pkg_rows.append({"Categoria": STAT_LABELS[c], "Tipo": "Saindo", "Valor": out_sum[c]})
        pkg_rows.append({"Categoria": STAT_LABELS[c], "Tipo": "Entrando", "Valor": in_sum[c]})
    pkg_long = pd.DataFrame(pkg_rows)
    if pkg_long.empty:
        st.info("Sem dados para o gráfico do pacote.")
    else:
        st.dataframe(pkg_long, use_container_width=True)
